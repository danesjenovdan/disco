...
COPY --chown=wagtail:wagtail ./disco .

# Copy compiled css from other image
COPY --chown=wagtail:wagtail --from=sass-compile /app/static/css ./disco/static/css

# Use user "wagtail" to run the build commands below and the server itself.
USER wagtail

CMD gunicorn disco.wsgi:application -b 0.0.0.0:8000 --log-level DEBUG
